...
**Key behaviors:**
- **Pattern matching** (`messageCreate.ts`) — Iterates `patterns.json` in order and fires **every** match, not just the first. A message saying both "hello" and "meat" gets both reactions, and a `"reply"` pattern lands alongside them. Reactions are deduplicated across the whole message (patterns *and* the encounter), since Discord ignores a repeat of the same emoji from the same user. Each action is try/caught individually, so one emoji the bot lacks permission for cannot suppress the rest.
...
- **Random encounters** (`encounters.ts`) — Rolls `chance` (default 2%) per message. `rollEncounter()` returns an **array** of actions: the emoji and the message are rolled independently, so one appearance can react *and* speak. If both independent rolls miss, one form is chosen at random so a hit is never silent and neither form is favoured. The `chance` gate is unchanged, so Krampus appears exactly as often as before — he can just do more when he does. `{user}` → mention.
