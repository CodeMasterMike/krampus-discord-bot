...
- **Score tracking** (`scoretracker.ts`) — Config-driven score parser (`scoretrackers.json`) for daily-puzzle games like putt.day. On each message, runs every tracker's regex; the first submission of a given round number per user is logged to `data/scores-data.json` (later re-posts of the same round are ignored) and confirmed with a quiet reaction. Handicap = average missed putts (`max - score`); place finishes (🥇🥈🥉) are derived live by ranking each round's participants (ties share a place; rounds need `minPlayersForMedal` players to count). "Today" = the highest round number recorded. Surfaced via `/putt-today`, `/putt-leaderboard`, `/putt-card`.
